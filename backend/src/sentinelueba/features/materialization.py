...
class FeatureMaterializer:
    ...
    def materialize(self, dataset_kind: str, *, rebuild: bool = False) -> dict[str, Any]:
        ...
        all_events = self.storage.list_event_rows(synthetic=synthetic)
        events = [row["event"] for row in all_events]
        all_observations = (
            self.storage.list_collector_observations() if dataset_kind == "real" else []
        )
        if not events and not all_observations:
            if rebuild:
        ...
        windows = self._build_windows(
            [row["event"] for row in all_events],
            dataset_kind=dataset_kind,
        ...
        self._update_state(
            dataset_kind,
            all_event_rows=all_events,
            observations=all_observations,
            rebuilt=rebuild,
            late_within=late_within,
            late_outside=late_outside,
        )
        watermark = max((event.timestamp for event in events), default=None)
        return {
            "dataset_kind": dataset_kind,
            "processed_events": len(new_events) if state and not rebuild else len(all_events),
            ...
            "event_time_watermark": watermark.isoformat() if watermark else None,
            "rebuild": rebuild,
    ...
    def _update_state(
        ...
        *,
        ...
        event_watermark = max(
            (row["event"].timestamp for row in all_event_rows),
            default=None,
        )
        ...
        self.storage.upsert_materialization_state(
            ...
            last_ingested_at=str(latest_event_row["ingested_at"]) if latest_event_row else None,
            last_event_id=latest_event_row["event"].event_id if latest_event_row else None,
            last_observation_at=last_observation_at,
            late_events_within_policy=late_within,
            late_events_outside_policy=late_outside,
            late_event_interval_minutes=self.late_event_minutes,
            window_size_minutes=self.window_minutes,
            baseline_state=self._baseline_state([row["event"] for row in all_event_rows]),
            rebuilt=rebuilt,
        )
