...
Schema initialization follows the normal v0-to-v9 migration chain. Fresh databases and historical v1-v8 fixtures migrate to v9. Schema v8 keeps Stage 2's v6 observation watermarking, Stage 3's model registry tables, and makes `model_promotions.new_model_id` nullable so retire audit rows store `NULL` instead of a fake empty successor. Schema v9 adds Stage 4 detection policy, evaluation, finding, suppression, watermark, and worker lease tables. A database already at v9 is checked for required tables and columns; missing required schema raises an integrity error instead of rerunning old migrations as self-healing DDL.
