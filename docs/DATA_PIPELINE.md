...
Schema initialization follows the normal v0-to-v10 migration chain. Fresh databases and historical fixtures migrate to v10. Schema v8 keeps Stage 2's v6 observation watermarking, Stage 3's model registry tables, and makes `model_promotions.new_model_id` nullable so retire audit rows store `NULL` instead of a fake empty successor. The first Stage 4 detection migration adds policy, evaluation, finding, suppression, watermark, and worker lease tables. Schema v10 adds persisted feature profile/input hashes, policy activation audit, expanded run status/counters, worker lease namespaces, suppression audit fields, and `finding-fingerprint-v2` correlation support. A database already at v10 is checked for required tables and columns; missing required schema raises an integrity error instead of rerunning old migrations as self-healing DDL.
